...
class Setting {
    ...
    public function edit($oldkey,$newkey){

        //xu ly chuoi :
        $check= explode(".",$oldkey);
        if(empty($check[1])){
            
            $newname=$newkey; //group check
            $flag=1;
        }else{
              $newname = $check[0].'.'.$newkey; //type check
              $flag=2;
        }
    
        $array = $this->get($oldkey);
 
        array_forget($this->settings,$oldkey);
         
         array_set($this->settings,$newname,'');
      
         if($flag==1){
              $this->settings[$newkey] = $array;
          }else{
             $this->settings[$check[0]][$newkey] = $array;
          }
    ...
    public function save($path = null, $filename = null)
    {
        ...
        fwrite($fh, json_encode($this->settings, JSON_PRETTY_PRINT));
        fclose($fh);
    }
